    ...
    public Map<String, Object> getOrderCount(String sql) {
        try {
            ...
            for(Map<String,Object> map : list) {
                ...
                // 查找日期列（可能的列名：order_date, date, orderDate, ORDER_DATE等）
                for (String key : map.keySet()) {
                    String lowerKey = key.toLowerCase();
                    if (lowerKey.contains("date") || lowerKey.contains("time") || lowerKey.contains("month") || lowerKey.contains("day")) {
                        dateValue = map.get(key);
                        break;
                    }
                }
                
                // 查找数量列（可能的列名：order_count, count, orderCount, ORDER_COUNT等）
                for (String key : map.keySet()) {
                    String lowerKey = key.toLowerCase();
                    if (lowerKey.contains("count") || lowerKey.contains("num") || lowerKey.contains("total")) {
                ...
                // 如果没找到，尝试使用第一个和第二个值
                if (dateValue == null && countValue == null && !map.isEmpty()) {
            ...
            log.info("订单统计数据处理完成，dateList大小: {}, countList大小: {}", 
                    dateList.size(), countList.size());
            
            return dataMap;
        } catch (Exception e) {
            // 记录错误日志
            String sqlPreview = sql != null && sql.length() > 200 ? sql.substring(0, 200) + "..." : sql;
            log.error("执行订单统计SQL失败，SQL: {}", sqlPreview, e);
            ...
            if (e instanceof org.springframework.jdbc.BadSqlGrammarException || 
                e.getCause() instanceof org.springframework.jdbc.BadSqlGrammarException) {
                errorMessage = "SQL语法错误，请检查生成的SQL语句";
            } else if (e instanceof java.sql.SQLException) {
                errorMessage = "数据库执行错误: " + errorMessage;
            }
